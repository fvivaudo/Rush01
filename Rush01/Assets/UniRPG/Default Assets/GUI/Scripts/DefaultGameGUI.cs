...
public class DefaultGameGUI : MonoBehaviour 
{
	...
	void Awake()
	{
		...
		if (!UniRPGGlobal.Instance) UniRPGGlobal.LoadLevelAdditive("unirpg");
	}
#endif
	...
	void OnDestroy()
	{
		if (InputManager.InstanceExist) InputManager.Instance.UnloadInputBinder(new DefaultGUIInputBinder(), OnKeybindOrderChanged);
