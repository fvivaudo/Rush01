...
namespace UniRPG
{
	...
	public class Chara2_Player : Chara2_Base
	{
		...
		{
			inited = true;
			InputManager.Instance.LoadInputFromBinder(new Chara2_Player_Input(), OnKeybindOrderChanged);
		}
		...
		{
			base.OnDestroy();
			if (InputManager.InstanceExist) InputManager.Instance.UnloadInputBinder(new Chara2_Player_Input(), OnKeybindOrderChanged);
		}
				...
				if (floorLayerMask.HasLayer(hits[i].transform.gameObject.layer))
				{
					...
					if ((hits[i].point - _tr.position).sqrMagnitude >= minClickDistance * minClickDistance)
					{
						if (clickMarker)
						{
							...
							if (clickMarker.GetComponent<Animation>()) clickMarker.GetComponent<Animation>().Play();
						}
