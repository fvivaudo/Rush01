...
public class DefaultCam1 : GameCameraBase 
{
	...
	public bool allowZooming = true;			// allow zooming the camera toward/from the target. (note that global setting in DB can override this to false)

	public float distance = 8f;					// distance of camera from character (influenced by zoom)
	public float backDistance = 0.85f;			// how far back from the character the cam should be in relation to "distance"
	...
	public override void ActivatedByUniRPG() 
	{
		...
		InputManager.Instance.LoadInputFromBinder(inputBinder, OnKeybindOrderChanged);
	}
	...
	public override void DeactivatedByUniRPG()
	{
		...
		InputManager.Instance.UnloadInputBinder(inputBinder, OnKeybindOrderChanged);
	}

	private void OnKeybindOrderChanged()
	{
	}
	...
	public void OnInput_UpdateRotation(int dir)
	{
		if (allowRotation)
		{
			...
			if (dir == 0) targetAngle += Input.GetAxis("Mouse X") * rotationStepping;
			else targetAngle += dir * (rotationStepping / 3f);
