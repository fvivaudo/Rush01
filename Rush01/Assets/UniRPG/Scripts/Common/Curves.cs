// From, http://whydoidoit.com/2012/04/06/unity-curved-path-following-with-easing/
...
public static class Spline
{
	...
	{
		// Leslie - I do not want Linq in UniRPG Core so I changed this.
		//return (new Vector3[] { path[0] }).Concat(path).Concat(new Vector3[] { path[path.Length-1]}).ToArray();
		Vector3[] r = new Vector3[path.Length+2];
		r[0] = path[0];
		for (int i = 0; i < path.Length; i++) r[i + 1] = path[i];
		r[r.Length-1] = path[path.Length-1];
		return r;
	}
