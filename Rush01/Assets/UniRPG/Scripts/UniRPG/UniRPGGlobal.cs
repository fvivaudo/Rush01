...
public class UniRPGGlobal : MonoBehaviour
{
	...
	public enum State { 
		...
		InMainMenu,		// the player is in the Main Menu
	...
	public void LoadNewGame()
	{
		...
		if (DB.gameSceneNames.Count > 0)
		{
			...
			UniRPGGlobal.LoadLevel(DB.gameSceneNames[0]);
		}
		else
		{
	...
	private void RunAutoCalls(List<string> classNames)
	{
		for (int i=0; i < classNames.Count; i++)
		{
			...
			//if (c == null) c = UnityEngineInternal.APIUpdaterRuntimeServices.AddComponent(gameObject, "Assets/UniRPG/Scripts/UniRPG/UniRPGGlobal.cs (496,23)", classNames[i]);
			if (c == null) c = gameObject.AddComponent(UniRPGUtil.FindType(classNames[i]));
	...
	public static void DeleteAllSaveSlots()
	{
		if (DB.loadSaveProviderPrefab)
		{
			...
			if (loadSaveProvider != null)
			{
				if (_instance != null) _instance.SaveSlots = new Dictionary<string, string>(0);
				loadSaveProvider.DeleteAll();
			}
		}
	}
	...
	void Update()
	{
		...
		{
			if (!UniRPGGlobal.isLoadingLevel)
			{	// done loading the Menu Data, now load the actual main menu scene
				...
				UniRPGGlobal.LoadLevelAdditive("menugui");
			}
		}
	...
	// a hack since Application.isLoading is deprecated. It depends on this flag being set/reset whenever Application.Load... is used
