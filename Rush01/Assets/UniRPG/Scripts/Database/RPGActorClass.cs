...
public class RPGActorClass : ScriptableObject
{
	...
	public void SaveState(string key)
	{
		...
		{
			UniRPGGlobal.SaveString(key + "att_" + i, 
				Attributes[i].id.ToString() + "|" +				// 0
				Attributes[i].MinValue.ToString() + "|" +		// 1
				Attributes[i].NoBonusMax.ToString() + "|" +		// 2
				Attributes[i].NoBonusValue.ToString() + "|" +	// 3
				Attributes[i].Bonus.ToString() + "|" +			// 4
				Attributes[i].BonusMax.ToString());				// 5
		}
	}
			...
			if (!string.IsNullOrEmpty(v))
			{
				...
				if (vs.Length >= 4)
				{
					...
					for (int j = 0; j < Attributes.Count; j++)
					{
						...
						{
							bool good = true;
							float val = 0f, min = 0f, max = 0f, bonus = 0f, bonusMax = 0f;
							...
							if (vs.Length > 5)
							{
								if (!float.TryParse(vs[5], out bonusMax)) good = false;
							}
							if (good)
							{
								failed = false;
								Attributes[j].SetWithoutTriggeringEvents(val, min, max, bonus, bonusMax);
								if (xpAttrib != null)
								{
