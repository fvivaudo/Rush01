...
public class CurrencyAction : Action
{
	...
	public override ReturnType Execute(GameObject self, GameObject targeted, GameObject selfTargetedBy, GameObject equipTarget, GameObject helper)
	{
		...
		if (obj != null)
		{
			...
			if (actor)
			{
				if (doWhat == 0) actor.currency = (int)amount.Value(self, targeted, selfTargetedBy, equipTarget, helper);
				else if (doWhat == 1) actor.currency += (int)amount.Value(self, targeted, selfTargetedBy, equipTarget, helper);
				else if (doWhat == 2) actor.currency -= (int)amount.Value(self, targeted, selfTargetedBy, equipTarget, helper);
			}
