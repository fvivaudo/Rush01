...
public class DebugLogAction : Action
{
	...
	public override ReturnType Execute(GameObject self, GameObject targeted, GameObject selfTargetedBy, GameObject equipTarget, GameObject helper)
	{
		...
		switch (logType)
		{
			case 0: Debug.Log(text.Value(self, targeted, selfTargetedBy, equipTarget, helper) + " " + (inclNum ? num.Value(self, targeted, selfTargetedBy, equipTarget, helper).ToString() : "") + o); break;
			case 1: Debug.LogWarning(text.Value(self, targeted, selfTargetedBy, equipTarget, helper) + " " + (inclNum ? num.Value(self, targeted, selfTargetedBy, equipTarget, helper).ToString() : "") + o); break;
			case 2: Debug.LogError(text.Value(self, targeted, selfTargetedBy, equipTarget, helper) + " " + (inclNum ? num.Value(self, targeted, selfTargetedBy, equipTarget, helper).ToString() : "") + o); break;
		}
