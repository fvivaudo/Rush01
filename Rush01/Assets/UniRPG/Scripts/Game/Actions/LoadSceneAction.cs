...
public class LoadSceneAction : Action
{
	...
	public override ReturnType Execute(GameObject self, GameObject targeted, GameObject selfTargetedBy, GameObject equipTarget, GameObject helper)
	{
		...
		if (loadBy == 0)		// from DB
		{
			UniRPGGlobal.Instance.playerSpawnPoint = (int)num.Value(self, targeted, selfTargetedBy, equipTarget, helper);
		...
		{
			UniRPGGlobal.Instance.LoadGameScene(sceneName.Value(self, targeted, selfTargetedBy, equipTarget, helper));
		}
