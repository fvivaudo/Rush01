...
public class AttributeAction : Action
{
	...
	public int affectedPart = 0; // 0: the Value, 1:Bonus, 2: Min, 3: Max, 4: BonusMax
	...
	public override ReturnType Execute(GameObject self, GameObject targeted, GameObject selfTargetedBy, GameObject equipTarget, GameObject helper)
	{
		...
		// ...
		float value = val.Value(self, targeted, selfTargetedBy, equipTarget, helper);
		if (useRandomRange)
		{
			float value2 = val2.Value(self, targeted, selfTargetedBy, equipTarget, helper);
		...
		if (valueMod.Count > 0)
		{
			...
			{
				value = ApplyValueMod(valueMod[i], value, actor, aggActor, subB, aggB, self, targeted, selfTargetedBy, equipTarget, helper);
			}
		}
		...
		else if (affectedPart == 4)
		{	// to the BonusMax
			if (doWhat == 0) attrib.BonusMax -= value;		// subtract
			else if (doWhat == 1) attrib.BonusMax += value;	// add
			else if (doWhat == 2) attrib.BonusMax = value;	// set
		}
		return ReturnType.Done;
	}
