...
public class RPGActorClassEditor : DatabaseEdBase
{
	...
	private void AttributeValues()
	{
		if (selAttrib != null)
		{
			EditorGUILayout.BeginHorizontal();
			{
				EditorGUILayout.BeginVertical();
				{
					UniRPGEdGui.LookLikeControls(135, 30);
					...
					EditorGUILayout.Space();
					UniRPGEdGui.LookLikeControls(150, 20);
	...
	private bool ContainAttribType(List<RPGAttributeData> l, UniRPG.GUID id)
	{
	...
	private void LevelingSettings()
	{
		...
		EditorGUILayout.BeginHorizontal();
		{
			UniRPGEdGui.LookLikeControls(135, 80);
			EditorGUI.BeginChangeCheck();
			...
			{
				curr.xpAttribId = new UniRPG.GUID();
				CheckXPAttrib();
			}
		}
		EditorGUILayout.EndHorizontal();
