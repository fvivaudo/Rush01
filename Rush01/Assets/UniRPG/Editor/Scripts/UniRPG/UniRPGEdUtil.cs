...
public class UniRPGEdUtil 
{
	...
	public static GameObject CreateGameObjectInSceneView(string name, Vector3 offset, bool checkColliders, int mask)
	{
		return CreateGameObjectInSceneView(name, offset, checkColliders, mask, null);
	}
	...
	public static GameObject CreateGameObjectInSceneView(string name, Vector3 offset, bool checkColliders, int mask, GameObject prefab)
	{
		GameObject obj = prefab == null ? new GameObject(name) : (GameObject)Object.Instantiate(prefab);
		obj.name = name;

		// position the object around where scene view cam is looking
		// first attempt to find colliders if checkColliders=true, and use the given mask if needed
		SceneView sv = SceneView.currentDrawingSceneView != null ? SceneView.currentDrawingSceneView : SceneView.lastActiveSceneView;
		if (sv != null)
		{
			Vector3 pos = Vector3.zero;
