...
public class UniRPGEditorGlobal
{
	...
	public static void Create_SpawnPoint()
	{
		...
		GameObject obj = UniRPGEdUtil.CreateGameObjectInSceneView("SpawnPoint", new Vector3(0f, 0.05f, 0f), true, (1 << UniRPGSettings.floorLayeMask));
		if (!obj) return;
		obj.transform.position += new Vector3(0f, 0.08f, 0f); // want it a little higher
	...
	public static void Create_PatrolPath()
	{
		...
		GameObject obj = UniRPGEdUtil.CreateGameObjectInSceneView("PatrolPath " + (parent.transform.childCount + 1).ToString(), new Vector3(0f, 0.05f, 0f), true, (1 << UniRPGSettings.floorLayeMask));
		if (!obj) return;
		obj.transform.parent = parent.transform;
	...
	public static void Create_Trigger()
	{
		...
		GameObject obj = UniRPGEdUtil.CreateGameObjectInSceneView("Trigger " + (parent.transform.childCount + 1).ToString(), new Vector3(0f, 0.05f, 0f), true, (1 << UniRPGSettings.floorLayeMask));
		if (!obj) return;
		obj.transform.parent = parent.transform;
