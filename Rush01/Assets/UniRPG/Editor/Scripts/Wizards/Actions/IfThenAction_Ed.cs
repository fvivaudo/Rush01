...
public class IfThenAction_Ed : ActionsEdBase
{
	...
	private static readonly string[] VarTypeStrings1 = { "Numeric", "Text", "Object", "Attribute", "AttribMin", "AttribMax", "Level", "Currency", "CustomVar", "Subject", "BagContain" };
	private static readonly string[] VarTypeStrings2 = { "Numeric", "Text", "Object", "Attribute", "AttribMin", "AttribMax", "Level", "Currency", "CustomVar", "Subject", "Empty (null)", "IsActor", "IsPlayer", "Enabled", "Friendly", "Neutral", "Hostile", "Item" };
		...
		scroll = UniRPGEdGui.BeginScrollView(scroll, UniRPGEdGui.ScrollViewNoMarginPaddingStyle, GUILayout.Height(120));
		{
			...
			foreach (IfThenActionTest t in action.tests)
			{
				EditorGUILayout.BeginHorizontal();
				{
					...
					{
						int sel = (int)t.varType[0];
						if (t.varType[0] == IfThenActionTest.VarType.BagContain) sel = 10;

						sel = EditorGUILayout.Popup(sel, VarTypeStrings1, GUILayout.Width(85));

						if (sel == 10) t.varType[0] = IfThenActionTest.VarType.BagContain;
						else t.varType[0] = (IfThenActionTest.VarType)sel;
						...
						// attribute, level, currency, subject
									...
									t.varType[0] == IfThenActionTest.VarType.BagContain ||
					...
					EditorGUILayout.BeginVertical();
					{
						...
						else if (t.varType[1] == IfThenActionTest.VarType.Item)
						{
							string nm = "-select-";
							RPGItem it = UniRPGEditorGlobal.DB.GetItem(t.attribId[1]);
							if (it != null) nm = it.screenName;
							if (GUILayout.Button(nm, EditorStyles.miniButton, GUILayout.Width(85)))
							{
								ItemSelectWiz.Show(false, OnItemSelected, new object[] { action, t });
							}
						}
	...
	private void OnItemSelected(object sender, object[] args)
	{
		ItemSelectWiz wiz = sender as ItemSelectWiz;
		IfThenAction act = (IfThenAction)args[0];
		IfThenActionTest test = (IfThenActionTest)args[1];
		if (act != null && test != null)
		{
			test.attribId[1] = wiz.selectedItems[0].prefabId;
