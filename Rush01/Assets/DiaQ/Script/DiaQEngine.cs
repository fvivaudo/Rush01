...
namespace DiaQ
{
	...
	public class DiaQEngine : MonoBehaviour
	{
		...
		/// <summary>
		/// Find defined quest by build-in ident, as generated when graph was defined
		/// </summary>
		public DiaQGraph FindDefinedGraph(string ident)
		{
		...
		public string GetSaveData()
		{
			...
			// save data related to graphs
			data.Append("@||@");
			foreach (DiaQGraph g in asset.graphs)
			{
				data.Append(g.IdentString).Append(":").Append(g.HasSeenPlayer ? "1" : "0").Append(";");
			}
		...
		/// </summary>
		public void RestoreViaSaveData(string dataIn)
		{
			if (asset == null) return;
			if (string.IsNullOrEmpty(dataIn)) return;
			string[] dataSplit = dataIn.Split(new string[] { "@||@" }, System.StringSplitOptions.None);

			if (dataSplit.Length < 1)
			{
				Debug.LogError("DiaQ Save Data not in correct format (0).");
				return;
			}
			...
			if (dataSplit.Length > 1)
			{
				string[] graphData = dataSplit[1].Split(';');
				foreach (string gds in graphData)
				{
					if (string.IsNullOrEmpty(gds)) continue;
					string[] gd = gds.Split(':');
					DiaQGraph g = FindDefinedGraph(gd[0]);
					if (g != null || gd.Length < 2)
					{
						g.HasSeenPlayer = gd[1] == "1";
					}
					else Debug.LogError("DiaQ Save Data not in correct format (7).");
				}
			}
