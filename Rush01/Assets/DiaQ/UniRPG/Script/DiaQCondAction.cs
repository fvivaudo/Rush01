...
namespace DiaQ
{
	...
	public class DiaQCondAction : Action
	{
		...
		public override ReturnType Execute(GameObject self, GameObject targeted, GameObject selfTargetedBy, GameObject equipTarget, GameObject helper)
		{
			if (setWhat == 0)
			{
				DiaQEngine.Instance.UpdateQuestConditions(conditionKey.Value(self, targeted, selfTargetedBy, equipTarget, helper), (int)value.Value(self, targeted, selfTargetedBy, equipTarget, helper));
			}
			else
			{
				DiaQuest q = DiaQEngine.Instance.FindAcceptedQuest(questIdent);
				if (q != null) q.UpdateCondition(conditionKey.Value(self, targeted, selfTargetedBy, equipTarget, helper), (int)value.Value(self, targeted, selfTargetedBy, equipTarget, helper));
			}
